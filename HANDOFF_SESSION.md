...
- **จุด offer BLK 18 ก.ย.** — ยังไม่ได้ใส่ลงหน้า เพราะเป็นข้อมูลราคาคู่แข่ง/ราคาเสนอของเราเอง ส่วน repo นี้เป็นสาธารณะ (login gate อยู่ฝั่ง client เท่านั้น) · ตัวเลขอยู่ในชีตของทีม (แท็บแรก ตาราง 'ราคา BL/BLK, BB, FL ของบราซิลจากลูกค้า') ถ้าจะใส่ต้องยืนยันก่อน
...
### Tab 10 — ฮ่องกงเทียบปี 2568 (22 ก.ย. 2569)
- เพิ่ม `HK25F/HK25C` → กราฟฮ่องกงเป็น 4 ชุด (สด 2568/2569 · ปรุงสุก 2568/2569) สีอ่อน=ปีก่อน สีเข้ม=ปีนี้
- ที่มา: **ม.ค.–ส.ค. 2568 ใช้คอลัมน์เปรียบเทียบในรายงานปี 2569** (ฉบับล่าสุดของแต่ละงวด = ค่าที่ revise แล้ว)
  · **ก.ย.–พ.ย. 2568 ใช้รายงานปี 2568** (Packer18/20/21_2568) — หักลบจากฐานของชุดเดียวกันเพื่อไม่ให้ปนการ revise
  · **ธ.ค. 2568 = ไม่มี** เพราะไม่มีรายงานสรุป Jan-Dec (ฉบับ ม.ค.2569 ให้ถึง พ.ย. เท่านั้น)
- 2568 รายเดือน · สด: 842,689,858,654,927,830,859,890,1036,1304,1084 · ปรุงสุก: 486,735,558,434,772,790,659,771,579,1147,599
- **`fmtK()` + y-axis ปรับเกณฑ์จาก 1,000 เป็น 10,000** — เดิมค่าหลักพันกลายเป็น "1k" หมด (ฮ่องกง/UK อ่านไม่ได้) ตอนนี้ต่ำกว่า 10,000 โชว์เลขเต็มมีลูกน้ำ · กราฟอื่น (หลักหมื่น+) เหมือนเดิม

### Tab 10 — รื้อใหม่: 7 ตลาด × 3 ปี + ปุ่มเลือกปี (22 ก.ย. 2569)
**`extract_export_markets.py` (ใหม่)** — สกัด Table 2 จาก Packer PDF ทุกฉบับ (49 ไฟล์ 2567–2569) → `export_markets.json`
- อ่านด้วย `page.extract_words()` + **ต่อคำที่ห่างกัน <5px** เพราะ pdfplumber แตกตัวเลขเป็น '1' + '34,142' (ใช้ extract_text แล้วต่อ string จะพังเพราะไปต่อเลขคนละคอลัมน์เข้าด้วยกัน)
- ชื่อแถวก็โดนต่อคำ ('- JAPAN' → '-JAPAN') → เทียบแบบตัดช่องว่างทิ้ง
- **ตรวจความถูกต้องในตัว**: ต้องมี สด + ปรุงสุก = รวม (±2 ตัน) ไม่งั้นข้ามแถวนั้น
- งวด [Jan] (เดือนแรก) ต้องจับแยกจาก [Jan-Aug] ไม่งั้น ม.ค./ก.พ. หายทุกปี
- ผลลัพธ์: **2567 ครบ 12 เดือน · 2568 11 เดือน (ไม่มี ธ.ค.) · 2569 8 เดือน (ถึง ส.ค.)** — ยอดรวมตรงกับ Table 2 ทุกตลาด
**UI**: ปุ่มปี `setExY()` — 2567 / 2568 / 2569* / เทียบ 3 ปี (ยอดรวมสด+ปรุงสุก 3 ปีซ้อน) · KPI 7 การ์ดคำนวณเอง (`_exKpi`) รวม %YoY ที่**เทียบเฉพาะเดือนที่มีข้อมูลทั้งสองปี**
- เพิ่ม 2 ตลาดใหม่: **เกาหลีใต้** (−22.7%) · **จีน** (−56.8%, ไก่สดล้วน) · `.kpi-row.kauto` = grid auto-fit
- ตัวแปร `JPF/JPC/EUF/...` เดิมถูกแทนด้วย `EXM` (ฝังใน index.html) — อัปเดตงวดหน้าให้รัน `extract_export_markets.py` แล้ววางบล็อก `var EXM={...}` ใหม่

#### ปรับ Tab 10: ปุ่มปี "แยกทีละตลาด" (ตามที่ผู้ใช้ขอ)
- `EXM_Y` เป็น object {ตลาด: ปี} · `setExY(mk, y)` รับตลาดเป็นพารามิเตอร์แรก · ปุ่มอยู่ในการ์ดแต่ละใบ (id `exy-<mk>-<ปี>`)
- การ์ด KPI แยก id `kpi-ex-<mk>` → อัปเดตเฉพาะใบนั้นเมื่อเปลี่ยนปี (ไม่ rebuild ทั้งแถว)
- ⚠ ตอนแก้เคยลืมนิยาม `_thY()` ใหม่หลังเขียนบล็อก JS ทับ → setExY โหมด "เทียบ 3 ปี" พังเงียบๆ (ปุ่มเปลี่ยนสีแต่กราฟไม่เปลี่ยน) · ถ้าเจออาการนี้อีกให้ดู ReferenceError ใน console ก่อน

### 🔄 อัปเดตข้อมูลญี่ปุ่น ก.ค. 2026 (24 ก.ย. 2569)
โฟลเดอร์ใหม่ `ข้อมูลญี่ปุ่น/ข้อมูลญี่ปุ่น update JUL 2026/` — **ไฟล์ MAFF เปลี่ยนมาใช้ชื่อรหัสแทนชื่อไทย**
`4010a`=สต็อก/อุปทาน · `4021a`,`4021aD`=ปริมาณนำเข้าแยกชิ้นส่วน · `4022a`=ราคานำเข้า CIF · `4023a`=นำเข้าแยกแหล่ง · `4031a`=ราคาขายส่งโตเกียว
- โครงตารางเหมือนเดิมทุกไฟล์ → parser เดิมอ่านได้เลย แค่เพิ่ม pattern ชื่อไฟล์
- ค่าใหม่เดือน ก.ค. 2026: **นำเข้าญี่ปุ่น บราซิล 39,977 · ไทย 17,467 · สหรัฐ 1,029 · อื่นๆ 6 ตัน** (รวม 58,478) · **สต็อกปลายเดือน 165,659 ตัน** · **CIF แช่แข็งรวม 499 เยน/กก.** (จาก 477) · ราคาโตเกียวสะโพก 836 / อก 479 (เท่าที่มีอยู่แล้ว)
- index.html: เติม ก.ค. ใน `BRA/THAI/USA/OTH` 2026 และ `ST_TOT26` (ค่าจากชีตจะทับเองเมื่อทีมกรอก) · `ST_IMP26` ยังว่างเพราะไฟล์ MAFF ไม่ได้แยกสต็อกนำเข้า/ในประเทศ
